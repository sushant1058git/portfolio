...
function renderProfile(p) {
  ...
  if (subtitle) subtitle.textContent = '> ' + (p.role || 'Software Engineer') + '_';
  ...
  if (p.resume_url) {
    ...
    if (resumeBtn) {
      ...
      resumeBtn.download = 'Sushant_Sinha_Resume.pdf';
    }
  }
...
function renderSkills(skills) {
  ...
  if (!grid) return;
  if (!skills.length) { grid.innerHTML = '<p style="color:var(--text-dim)">No skills found.</p>'; return; }
...
function renderExperience(exps) {
  ...
  if (!tl) return;
  if (!exps.length) { tl.innerHTML = '<p style="color:var(--text-dim)">No experience found.</p>'; return; }
...
function renderBlog(posts) {
  ...
  if (!grid) return;
  if (!posts.length) {
    grid.innerHTML = '<p style="color:var(--text-dim);font-family:\'Share Tech Mono\',monospace">No posts yet.</p>';
    return;
  }
...
if (contactForm) {
  contactForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (res.ok) { showToast("✓ Message sent! I'll reply soon."); this.reset(); }
